// src/api/services/auth.ts
import apiClient from '../client';
import { ApiResponse, LoginResponse } from '../../types/api';

export const authApi = {
  login: async (credentials: any) => {
    return apiClient.post<any, LoginResponse>('/auth/login', credentials);
  },

  register: async (customerData: any) => {
    return apiClient.post<any, ApiResponse>('/auth/register', customerData);
  },

  verifyEmail: async (data: { email: string; otp: string }) => {
    return apiClient.post<any, LoginResponse>('/auth/verify-email', data);
  },

  resendOtp: async (data: { email: string }) => {
    return apiClient.post<any, ApiResponse>('/auth/resend-otp', data);
  },

  googleLogin: async (credential: string, role?: string) => {
    return apiClient.post<any, LoginResponse>('/auth/google', { credential, role });
  },
};